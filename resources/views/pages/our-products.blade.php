...
@section('our-products') active @endsection
@section('title') আমাদের পণ্যসমূহ @endsection
@section('body') 
 ...
 <section id="breadcumbs">
           ...
           <div class="row">  
                <div class="col-lg-12 text-center"> 
                    <div class="bread-text wow fadeInUp">  
                          ...
                          <h4><a href="{{ url('/') }}">হোম</a><i class="fa fa-arrow-circle-right" aria-hidden="true"></i>আমাদের পণ্যসমূহ </h4>
                    </div>
                </div>       
           </div>
    </div>
  ...
  <section id="dealer">
      <div class="container">
        <div class="row">
            <div class="col-lg-3">
               <div class="dealer-category wow fadeInDown ml-1">
                  <h3>ক্যাটাগরিসমূহ </h3>
                  @foreach ($categories as $category)
                  <p><a href="{{ url('category/products/'.$category->id.'/'.$category->category_slug) }}" class="@yield($category->category_slug)">{{ $category->category_name }}</a></p>                    
                  @endforeach
               </div>
               
               <div class="dealer-category wow fadeInDown ml-1 " data-wow-delay="0.2s">
               ...
               <div class="dealer-brand wow fadeInDown ml-1" data-wow-delay="0.3s">
                  <h3>ডিলার</h3>
                  @foreach ($delars as $delar)              
                  <p><a href="{{ url('delar/products/'.$delar->id.'/'.$delar->company_slug) }}" class="@yield($delar->company_slug)">{{ $delar->company_name }}</a></p>
                  @endforeach
               </div>
               <div class="dealer-brand wow fadeInDown ml-1" data-wow-delay="0.3s">
                <h3>ডিলার পণ্যসমূহ</h3>
                @foreach ($delar_services as $service)              
                <p><a href="{{ url('delar/company-products/'.$service->id.'/'.$service->delar_product_slug) }}" class="@yield($service->delar_product_slug)">{{ $service->delar_product_name }}</a></p>
                @endforeach
             </div>
            </div>
            <div class="col-lg-9">
               <div class="row dealer-top">
                  <div class="col-lg-6 col-sm-4 dealers-header">
                     <div class="dealer-header">
                        <h3><span>{{ count($count) }}</span> টি পণ্য পাওয়া গেছে</h3>
                     </div>
                  </div>
                         ...
                         <form action="{{ route('search.products') }}" method="post">
                          @csrf
                            <div class="dealer-form">
                              <input type="text" name="search" placeholder="পণ্য খুজুন">
